...
bool PylosMove::operator<(const Board::Move &target) const
{
   const PylosMove &rhs = dynamic_cast<const PylosMove &>(target);
   int size = mLocs.size(), rSize = rhs.mLocs.size();

   if (mLocs[0] < rhs.mLocs[0])
      return true;
   else if (mLocs[0] > rhs.mLocs[0])
      return false;
   else {
      if (mType < rhs.mType)
         return true;
         ...
         return false;
		  ...
		  for (unsigned i = 1; i < PylosBoard::kSqr; i++) {
           ...
           // than the indexes that you can index into), compare this index.
			  if (size > i && rSize > i && mLocs[i] < rhs.mLocs[i])
				  return true;
			  else if (size > i && rSize > i && mLocs[i] > rhs.mLocs[i])
				  return false;
			  // If one of them is smaller than the other, cut off here
			  else if (size <= i && rSize > i)
				  return true;
			  else if (size > i && rSize <= i)
				  return false;
         }
...
void PylosMove::operator=(const string &src) {
   ...
   if (!strcmp(wd1, "Play")) {
      ...
      if (res == kPlayOne && brack1 == ']' && wd1[0] == '\0') {
      // [Staley] Fill in temp
         temp.push_back(p1);
      } else if (res == kPlayTwo && brack1 == ']' && brack2 == ']'
       && string(wd1).compare("taking") == 0 && wd2[0] == '\0') {
         temp.push_back(p1);
         temp.push_back(p2);
      } else if (res == kPlayThree && 
      ...
      res = sscanf(src.c_str(), " Promote from [ %hd , %hd ] to [ %hd , %hd %c "
       "%6s [ %hd , %hd %c %3s [ %hd , %hd %c %1s", &p2.first, &p2.second, 
       &p1.first, &p1.second, &brack1, wd1, &p3.first, &p3.second, &brack2, 
       wd2, &p4.first, &p4.second, &brack3, wd3);
      ...
      // them to be.  The 'wd' immediately after the last expected token is
      // the one that needs to be == '\0'.
      if (res == kPromTwo && brack1 == ']' && wd1[0] == '\0') {
         temp.push_back(p1);
         temp.push_back(p2);
      } else if (res == kPromThree && brack1 == ']' && brack2 == ']'
        && string(wd1).compare("taking") == 0 && wd2[0] == '\0') {
         temp.push_back(p1); 
         temp.push_back(p2);
         temp.push_back(p3);
      } else if (res == kPromFour && brack1 == ']' && brack2 == ']'
